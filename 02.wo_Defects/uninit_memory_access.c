...
* Description: Defect Free Code to identify false positives to identify access to memory locations which are uninitialized
...
void uninit_memory_access_003 ()
{
	...
	{
		strcpy(str1, str2);
        printf("%s %s\n",str1,str2); /*Tool should not detect this line as error*/ /*No ERROR:Uninitialized Memory Access*/
        free(str1);
	}
}
...
void uninit_memory_access_009()
{
    ...
    if(uninit_memory_access_009_func_001(flag)==0)
	{
		for(i=0;i<10;i++)
		{
			for(j=0;j<10;j++)
			{
				uninit_memory_access_009_doubleptr_gbl[i][j] += 1; /*Tool should not detect this line as error*/ /*No ERROR:Uninitialized Memory Access*/
			}
...
{
	uninit_memory_access_010_func_001(1);
    if(uninit_memory_access_010_s_001_arr_gbl!=NULL)
    {
    	++uninit_memory_access_010_s_001_arr_gbl->data; /*Tool should not detect this line as error*/ /*No ERROR:Uninitialized Memory Access*/
...
void uninit_memory_access_014 ()
{
  int ret = 0;
	...
	if(p != NULL)
	{
		ret = p->b; /*Tool should not detect this line as error*/ /*No ERROR:Uninitialized Memory Access*/
	    free(p);
	    p= NULL;
	}
        sink = ret;
