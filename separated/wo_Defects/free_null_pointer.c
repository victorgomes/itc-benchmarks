...
static unsigned int min=min_buffer;
static unsigned int max=max_buffer;
...
void free_null_pointer_008 ()
{
	...
	strcpy(str1,str);
	free(str1);/* Tool should not detect this line as error */ /*No ERROR:Freeing a NULL pointer*/
	str1 = NULL;
}
...
void free_null_pointer_009 ()
{
	...
	while(1)
	{
	    ...
	    free(free_null_pointer_009dst);/* Tool should not detect this line as error */ /*No ERROR:Freeing a NULL pointer*/
	    free_null_pointer_009dst = NULL;
	    break;
	}
}
...
void free_null_pointer_010_func_003()
{
	...
	while(SET_MEM)
	{
	    ...
	    free_null_pointer_010_gbl_dst = NULL;/* Tool should not detect this line as error */ /*No ERROR:Freeing a NULL pointer*/
	    break;
	}
}
...
free_null_pointer_011_u_001 * free_null_pointer_011_func_002 ()
{
	...
	switch (flag)
	{
		case 1:
		{
			free(u);/* Tool should not detect this line as error */ /*No ERROR:Freeing a NULL pointer*/
			u= NULL;
			return u;
		}
		case 2:
...
void free_null_pointer_012 ()
{
    ...
    if (free_null_pointer_012_func_001(0) == ZERO && MAXIMUM ==1)
    {
    	if(flag == 10)
        {
    		free(ptr);/* Tool should not detect this line as error */ /*No ERROR:Freeing a NULL pointer*/
    	    ptr = NULL;
        }
    }
}
...
void free_null_pointer_013 ()
{
	...
	{
	    double * fptr = *fp2;
        free(fptr);/* Tool should not detect this line as error */ /*No ERROR:Freeing a NULL pointer*/
        fptr = NULL;
	}while(i>=0 && i<=1);
}
...
void free_null_pointer_014_func_003(int flag)
{
   ...
   if (flag ==1)
   {
	   for (i=0;i<1;i++)
	   {
		  ...
		  free(free_null_pointer_014_gbl_s);/* Tool should not detect this line as error */ /*No ERROR:Freeing a NULL pointer*/
	   }
   }
}
